...
int main(int argc, char *argv[])
{
    ...
    for (int iii = 0; iii < repeat_time; ++iii)
    {
        ...
        ofstream out(s + ".gssrw5", std::ios_base::app);
        out.precision(15);
        for (int i = 0; i < MOTIF5_NUM; ++i)
        {
            out << i << " " << ans[i] << endl;
    ...
    cout << "SSRW and NMSRE is writing to " << s << ".gssrw5 and " << s << ".gnrmsessrw5" << endl;
    gettimeofday(&realend, NULL);
    dur = (realend.tv_sec - realstart.tv_sec) + (double)(realend.tv_usec - realstart.tv_usec) / 1000000.0;
    printf("All Time:%f\n", dur); //count time
    string nmrse_file_nameg = graph_name + "_" + to_string(given_time) + "_" + to_string(jump_len) + "_" + to_string(repeat_time) + ".gnrmsessrw5";
    string nmrse_file_namec = graph_name + "_" + to_string(given_time) + "_" + to_string(jump_len) + "_" + to_string(repeat_time) + ".cnrmsessrw5";
    count_5gnmrse(graph_name, res, nmrse_file_nameg);
    count_5cnmrse(graph_name, res, nmrse_file_namec);
    return 0;
}
